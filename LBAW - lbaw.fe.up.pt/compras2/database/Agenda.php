...
class News {
  ...
  function update($id, $title, $body, $date, $author, $status) {
	...
	$errors["ndate"] = "Data maior que a data atual!";
      elseif (strpos($errmsg, 'Datetime field overflow')) 
  ...
  function delete($id) {
    global $dbh, $schema;
    try {
      $stmt = $dbh->prepare("DELETE FROM $schema.news WHERE nid = :id");
      $stmt->bindParam(':id', $id, PDO::PARAM_INT);
      $stmt->execute();
      //$count = $stmt->rowCount();
    }
  ...
  function getAllNewsAuthorLimit($limit, $offset) {
    ...
    try {
      ...
      $_SESSION["s_errors"]["generic"][] = "ERRO[29.0]: ".$e->getMessage();
      header("Location: ../../index.php");
      die;
     }
  }
  ...
  // get last n days
  function getLastNewsAuthor($n) {
    global $dbh, $schema;
    try {
      $days= $n-1;
      // prepare won't work
      $sql = "SELECT N.*, A.aname 
                FROM $schema.news N, $schema.authors A 
               WHERE N.nauthor = A.aid 
                 AND ndate BETWEEN current_date - ". $days ." AND current_date
            ORDER BY ndate";
      $stmt = $dbh->query($sql);
      // get array containing all of the result set rows 
      $result = $stmt->fetchall(PDO::FETCH_ASSOC);
      ...
      $_SESSION["s_errors"]["generic"][] = "ERRO[29.1]: ".$e->getMessage();
      header("Location: ../../index.php");
      die;
     }
  }
